...
impl SqlExporter {
    ...
    fn generate_update(
        ...
        let filtered_cols: Vec<(String, String)> =
            if let Some(ref update_only) = self.update_columns {
                update_cols
                    .into_iter()
        ...
        let set_clauses: Vec<String> = filtered_cols
            .iter()
            .map(|(col, val)| format!("{} = {}", self.dialect.quote_identifier(col), val))
            .collect();
    ...
    fn generate_upsert(
        ...
        match self.dialect {
            ...
            SqlDialect::PostgreSQL => {
                ...
                let conflict_columns: Vec<String> = self
                    .primary_keys
                    .iter()
            ...
            SqlDialect::SqlServer | SqlDialect::Oracle => {
                ...
                let match_conditions: Vec<String> = self
                    .primary_keys
                    .iter()
                    .map(|pk| {
...
impl Exporter for SqlExporter {
    fn export(&self, data: &ExcelData, output_path: &str) -> Result<()> {
        ...
        // 写入文件头注释
        writeln!(file, "-- Generated by excel-cli")?;
        writeln!(file, "-- Dialect: {:?}", self.dialect)?;
        writeln!(file, "-- Mode: {:?}", self.mode)?;
        writeln!(file, "-- Table: {}", self.table_name)?;
        writeln!(file, "-- Rows: {}", data.row_count())?;
        writeln!(file)?;
        ...
        if use_batch {
            ...
            for (idx, row) in data.rows.iter().enumerate() {
                ...
                if batch_values.len() >= self.batch_size || idx == data.rows.len() - 1 {
                    let sql =
                        self.generate_batch_insert(&self.table_name, column_names, &batch_values);
                    writeln!(file, "{}", sql)?;
                    writeln!(file)?;
                    batch_values.clear();
                }
...
mod tests {
    ...
    fn test_sql_dialect_quote() {
        ...
        assert_eq!(
            SqlDialect::PostgreSQL.quote_identifier("table"),
            "\"table\""
        );
    ...
    fn test_format_values() {
        ...
        assert_eq!(
            dialect.format_value(&CellValue::String("test".to_string())),
            "'test'"
        );
